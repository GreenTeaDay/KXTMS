...
router.get('/schedules', auth, async (req, res) => {
  try {
    const schedules = await Schedule.find({ customerId: req.user._id });
    res.send(schedules);
  } catch (e) {
    res.status(500).send();
  }
...
router.get('/schedules/:id', auth, async (req, res) => {
  ...
  try {
    const schedule = await Schedule.findOne({ _id, customerId: req.user._id });
    if (!schedule) {
...
router.patch('/schedules/:id', auth, async (req, res) => {
  ...
  try {
    const schedule = await Schedule.findOne({ _id: req.params.id, customerId: req.user._id });
    if (!schedule) {
      return res.status(404).send();
    }
...
router.delete('/schedules/:id', auth, async (req, res) => {
  try {
    const schedule = await Schedule.findOneAndDelete({ _id: req.params.id, customerId: req.user._id });
    if (!schedule) {
      res.status(404).send();
    }
    res.send(schedule);
